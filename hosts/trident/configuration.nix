...
}:
{
  ...
  # ---------------------------------------------------------------------------
  # gpwm session shell. Single source of truth: the home-manager gpwm module
  ...
  # These secrets are owned by the `agentic` / `poseidon` users, which only exist
  # while the agentic-loop and poseidon modules above are imported. Keep them
  # commented out in lockstep with those modules — sops-install-secrets fails
  # activation if it cannot resolve the owner.
