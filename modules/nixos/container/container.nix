...
in
{
  ...
  config = mkIf cfg.enable {
    hardware.nvidia-container-toolkit = {
      ...
      # Stock nixpkgs nvidia-container-toolkit (1.19.1+) already patches the
      # runtime-hook and ldconfig paths in its own postPatch, so the previous
      # git-src override / manual substituteInPlace is no longer needed.
    };
